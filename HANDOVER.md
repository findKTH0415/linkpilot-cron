...
| 플랫폼 화면 | 29% | **70%** | **0%** | 렌더 검증 완료. `api: null`, 서버 미배선, 인쇄 경로 미연결 |
...
- 검증 근거: `git ls-tree origin/main`, `npm test` 323건, 화면 headless 렌더
...
🔴 3건 · 🟠 5건 · 🟡 5건 · ⚪ 2건 — 이 중 **사양 불일치 3건(D-03~D-05)이
이번 교차검증에서 새로 나온 것**이다.

D-01(카카오 OAuth 규칙)·D-02(발송 시각)는 **본건 목적과 무관하여 범위 외로 분류**했다.
같은 이유로 CLAUDE.md §2 도 고치지 않는다.
