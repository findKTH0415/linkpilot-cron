...
| 최신 커밋 | `8586b7f` 가이드 필드 입력값 화면 추가 (+ 이후 교차검증 수정) |
| PR #3 | open · draft · `mergeable_state: clean` · 140파일 / +27,972 −938 · 35커밋 |
| PR #4 | open · draft · clean · 브랜치 `claude/ops-cron-alerts-and-deploy` (별개) |
| 테스트 | `npm test` → **361 pass / 0 fail** |
...
|---|--:|--:|--:|---|
| IM Agent 파이프라인 | 43% | **80%** | **0%** | 11 Agent + core 19모듈, end-to-end 실행 확인. 실키 스키마·VWorld 미검증 |
| 플랫폼 화면 | 29% | **75%** | **0%** | 렌더 검증 완료. 가이드 필드는 API 연결됨. 나머지는 `api: null`, 서버 미배선, 인쇄 경로 미연결 |
...
- 검증 근거: `git ls-tree origin/main`, `npm test` 361건, 화면 headless 렌더
