...
test('★★★ 짝이 어긋나면 화면에 사람 말로 뜬다', () => {
  ...
  /* ★ 앞 판은 「새로고침해 주십시오」를 재고 있었다. 이제 화면이 **한 번은 스스로**
   *   다시 받으므로(D-149), 이 띠는 그러고도 안 맞을 때만 뜬다 — 그때 사람이 할 일은
   *   새로고침이 아니라 **여덟 글자를 알려 주는 것**이다. 재는 것은 「할 일이 적혀
   *   있는가」이지 특정 낱말이 아니다. */
  assert.match(made[0].textContent, /알려 주시면|새로고침/, '무엇을 하면 되는지 말해야 한다');
  assert.match(made[0].textContent, /화면\(HTML\)이 옛 판일 가능성/,
    '어느 쪽이 옛것인지 거꾸로 말하면 엉뚱한 데를 고치러 간다 (실제로 거꾸로 말했다)');
});
