...
test('★★★ 미리보기를 **git 으로 재지 않는다** (헛울음이 난다)', () => {
  ...
  /* ★★ **글자 모양이 아니라 「무엇과 무엇을 대는가」를 본다** 〈2026-09-07〉.
     앞 판은 `const changed = made.filter` 라는 **철자**를 찾았다. 그래서 변수
     이름만 바뀌어도 빨개졌다 — 재려던 뜻은 그대로인데. 재려는 것은
     **다시 만든 것을 「만들기 전(커밋본)」과 대는가**이므로 그것을 본다. */
  assert.ok(/made\.filter\(\(f\) => sha\(f\) !== before\[f\]\)/.test(CODE),
    '다시 만들기 전후를 안 댄다 — 재려는 것은 「다시 만들면 달라지는가」다');
});

test('★★★ 두 번째로 다시 만든 것도 **커밋본**과 댄다 (첫 판과 대면 안 된다)', () => {
  /* ★★★ 왜 있나 〈2026-09-07 · 실제로 그렇게 짰다가 잡혔다〉.
   *
   *   미리보기 칸이 부하를 타 가끔 헛울음을 내서, 「다르면 한 번 더 만들어 보고
   *   판정」하도록 고쳤다. 그런데 처음 판은 **두 번째 판을 첫 번째 판과** 댔다.
   *   그러면 재는 것이 **만드는 도구가 같은 값을 내는가**뿐이라 —
   *   **커밋본이 진짜로 낡았어도 「흔들림」으로 삼켜진다.** 「고쳐 주고 넘어가는」
   *   바로 그것이 된다.
   *
   *   ★ 그래서 다시 만든 뒤의 비교 대상이 `before` 인지 못박는다. 첫 판을 담아 둔
   *     변수(`mid` 같은 것)와 대는 코드가 들어오면 이 칸이 빨개진다. */
  const i = CODE.indexOf('function previews()');
  const body = CODE.slice(i, CODE.indexOf('\n}', i));
  const j = body.indexOf('flaky');
  assert.ok(j > 0, '한 번 더 만들어 보는 갈래를 못 찾았다');
  const retry = body.slice(j, j + 700);
  assert.ok(/sha\(f\) !== before\[f\]/.test(retry),
    '★ 두 번째 판을 **커밋본(before)** 과 대지 않는다 — 진짜 어긋남이 흔들림으로 삼켜진다');
  assert.ok(!/!== (mid|first)\[f\]/.test(retry),
    '★ 첫 번째 판과 대고 있다 — 그것은 도구의 일관성만 재는 것이지 갈림을 재는 것이 아니다');
});
