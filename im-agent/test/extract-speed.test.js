...
/**
 * ★★★ **총 시간만 알면 다음에 무엇을 줄일지 정할 수가 없다** 〈2026-08-23〉.
 *
 *   화면은 「8분 0초」라고만 했다. 그 8분이 OCR 인지 LLM 보완인지 알 수가 없으니
 *   줄일 자리를 고르는 것이 짐작이 된다.
 *
 * ★ 재는 장치를 넣을 때는 **「나오면 다음에 무엇을 보는가」**를 함께 정한다
 *   (MEMORY M-30). 그래서 걸음마다 초를 남기고, 오래 걸린 것을 이름으로 말한다.
 */
test('★★★ 걸린 시간을 걸음별로 남기고, 오래 걸린 것을 이름으로 말한다', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'agents', '02-extraction.js'), 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
  assert.ok(/doc\.msOcr = msOcr/.test(src), 'OCR 에 걸린 시간을 안 남긴다');
  assert.ok(/doc\.msLlm = msLlm/.test(src), 'LLM 보완에 걸린 시간을 안 남긴다');
  assert.ok(src.indexOf('읽는 데 걸린 시간') !== -1, '합계를 한 줄로 안 말한다');
  assert.ok(src.indexOf('오래 걸린 것') !== -1,
    '어느 파일이 오래 걸렸는지 이름으로 안 말한다 — 줄일 자리를 짐작으로 고르게 된다');
  /* ★ 몇 개씩 읽었는지도 적는다 — 한도를 바꿔 보고 견주려면 이것이 있어야 한다 */
  assert.ok(/한 번에 \$\{CONCURRENCY\}개씩/.test(src), '몇 개씩 읽었는지를 안 적는다');
});
