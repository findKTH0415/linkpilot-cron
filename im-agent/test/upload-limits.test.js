...
/**
 * ★★★ **거절당했을 때 0 B 라고 말하지 않는다** 〈2026-08-22 · 실제 신고 화면〉.
 *
 * ★★ 신고 화면은 이렇게 적혀 있었다: 「이번에 보낸 양이 **0 B** 인데…」.
 *   그런데 실제로는 5MB 를 다 보내고 서버가 거절한 것이었다. 자취 그래프는
 *   4.6초 동안 끝까지 올라가 있었다.
 *
 * ★★ 0 은 **잰 값이 아니라 잃어버린 값**이었다. `upload-core.js` 의 HTTP 오류
 *   가지만 `bodyBytes` 를 빼고 상태를 갈아치웠기 때문이다. 화면에서는 「0 을
 *   쟀다」와 「못 쟀다」가 **똑같이 보인다** — 그 숫자를 믿고 「아무것도 안 갔다」
 *   로 원인을 찾으면 엉뚱한 자리를 판다.
 */
test('★★★ 서버가 거절해도 보낸 본문 크기를 잃지 않는다 (0 B 로 찍히던 자리)', () => {
  const up = fs.readFileSync(path.join(ROOT, 'ui/platform/upload-core.js'), 'utf8');

  /* 상태를 갈아치우는 자리를 모두 뽑아, **오류 가지들이 bodyBytes 를 들고 있는지**
     본다. 한 자리만 빼먹어도 그 경로에서만 0 이 되고, 그때는 아무도 못 알아챈다 */
  const errSays = (up.match(/say\(\{ phase: 'error'[^}]*\}\)/g) || []);
  assert.ok(errSays.length >= 3,
    `오류를 알리는 자리를 못 찾았다 (${errSays.length}개) — 검사가 아무것도 재지 못했다`);
  errSays.forEach((line) => {
    assert.match(line, /bodyBytes/,
      `오류 가지 하나가 bodyBytes 를 흘린다 — 그 경로에서만 「0 B」로 찍힌다: ${line}`);
  });

  /* ★ 화면 쪽도 확인한다. 못 쟀을 때 **0 B 라고 쓰지 않고 못 쟀다고** 말해야 한다 */
  const src = fs.readFileSync(path.join(ROOT, 'ui/platform/files.html'), 'utf8');
  assert.match(src, /보낸 양은 재지 못했습니다/,
    '못 쟀을 때도 숫자를 적는다 — 0 이 잰 값처럼 읽힌다');
  assert.match(src, /0 이라는 뜻이 아닙니다/,
    '못 쟀다는 말에 「0 이 아니다」를 안 붙였다 — 읽는 사람은 0 으로 받아들인다');
});

/**
 * ★★ **되는 것과 안 되는 것을 나눠 적는다** 〈2026-08-22 사용자 지시〉.
 *
 * 앞 판은 한 문단에 사실과 짐작을 몰아 적었다. 그러면 제목(「로그인이 필요합니다」)
 * 만 남아 엉뚱한 데를 고치러 간다.
 */
test('★★ 올리기가 거절당하면 되는 것·안 되는 것·할 일을 나눠 적는다', () => {
  const src = fs.readFileSync(path.join(ROOT, 'ui/platform/files.html'), 'utf8');
  ['되는 것 —', '안 되는 것 —', '할 일 —'].forEach((k) => {
    assert.ok(src.includes(k), `거절 안내에 「${k}」 가 없다`);
  });
  /* ★★ 〈2026-08-22 정정〉 앞 판은 「짐작 —」 문단을 요구했다. 그런데 그 짐작이
     실제로 **틀렸다** — 크기 문제라고 적어 두었는데 원인은 헤더였다. 화면이 짐작을
     적으면 사람은 그 짐작부터 판다. 그래서 요구하는 것을 바꾼다: **가른 사실**을
     적을 것, 그리고 **크기를 원인으로 단정하지 말 것.** */
  assert.match(src, /가른 사실 —/,
    '무엇이 되고 무엇이 안 됐는지를 가른 사실로 적지 않는다');
  assert.ok(!/짐작 —[\s\S]{0,120}본문[\s\S]{0,40}잘려/.test(src),
    '크기를 원인으로 짚는 짐작이 남아 있다 — 실제 원인은 헤더였다 (M-24)');
  /* ★ HTTP 상태를 적는다. 401 인지 403 인지에 따라 볼 자리가 다르다 */
  assert.match(src, /HTTP ' \+ \(uf\.status/, '거절 상태 코드를 안 적는다');
});
