...
test('B-2. Gate «번호» 겹침이 A-1 에 적혀 있다 (낱말이 아니라 숫자라 위 검사에 안 걸린다)', () => {
  /* ★ 2026-09-12 — 인사이트 지침서 v1.1 이 Gate 0~6 을 새로 두면서 **Gate 3** 이
     두 뜻을 갖게 됐다 (이쪽: Staging 실환경 · 저쪽: 콘텐츠·디자인).
     위 B 검사는 **대문자 상태 낱말**만 본다 — 숫자는 안 본다. 그래서 이 칸이 따로 있다.
     ★★ 재는 것은 「겹쳤는가」가 아니라 **「적어 두었는가」**다. 적힌 것이 지워지면
        다음 사람이 「Gate 3 통과」를 자기 뜻으로 읽는다. */
  const known = section(read(GUIDE), /같은 이름이 다른 뜻으로/);
  assert.ok(known, '부록 A-1 을 못 찾았습니다');
  assert.match(known, /Gate 3/,
    'A-1 에 Gate 번호 겹침이 없습니다 — 「Gate 3 통과」가 두 뜻을 갖는데 아무 데도 안 적혀 있습니다');
  for (const p of ['INSIGHT:', 'RELEASE:']) {
    assert.ok(known.indexOf(p) >= 0,
      'A-1 에 접두어 `' + p + '` 가 없습니다 — 부르는 법을 안 정하면 적어 둔 뜻이 없습니다');
  }
});

test('B-3. 「테마」·「다운로드」 겹침이 A-1 과 CLAUDE.md 둘 다에 적혀 있다', () => {
  /* ★ 2026-09-12 — 인사이트 지침서 v1.5 교차검증에서 나온 겹침 둘이다.
     위 B 검사는 **대문자 상태 낱말**만 보고, B-2 는 숫자만 본다 — 한글 낱말은 아무도 안 본다.

     ★★ 왜 이 둘이 위험한가.
       · 「테마」 — 지침은 문장 분류 60개, 코드(`_IMG_TAGS`)는 배경 검색어 51개다.
         숫자가 비슷해서 「60개로 맞춰라」가 이미지 쪽에서 읽히면 엉뚱한 것이 아홉 개 생긴다.
       · 「다운로드」 — 지침 §7.4 는 «완성 카드 저장»을 요구하고 사진 규칙 ③ 은
         «원본 사진 내려받기»를 금지한다. 한쪽이 요구사항, 한쪽이 금지사항이다.
     ★★★ 재는 것은 「겹쳤는가」가 아니라 **「부르는 법을 적어 두었는가」**다.
        접두어가 지워지면 다음 사람이 「다운로드 구현」을 자기 뜻으로 읽는다. */
  const known = section(read(GUIDE), /같은 이름이 다른 뜻으로/);
  assert.ok(known, '부록 A-1 을 못 찾았습니다');
  for (const w of ['테마', '다운로드']) {
    assert.ok(known.indexOf('`' + w + '`') >= 0,
      'A-1 표에 「' + w + '」 겹침이 없습니다 — 뜻이 둘인데 아무 데도 안 적혀 있습니다');
  }
  for (const pre of ['CONTENT:', 'IMAGE:', 'CARD:', 'PHOTO:']) {
    assert.ok(known.indexOf(pre) >= 0,
      'A-1 에 접두어 `' + pre + '` 가 없습니다 — 부르는 법을 안 정하면 적어 둔 뜻이 없습니다');
  }
  /* ★ CLAUDE.md 쪽에도 있어야 한다 — 작업할 때 읽는 것은 그쪽이다 */
  const md = read(path.join(ROOT, 'CLAUDE.md'));
  assert.ok(/CARD:다운로드/.test(md) && /PHOTO:다운로드/.test(md),
    'CLAUDE.md 에 `CARD:다운로드`·`PHOTO:다운로드` 가 없습니다 — '
    + '지침서에만 적으면 작업하는 자리에서 안 보입니다');
  assert.ok(/CONTENT:테마/.test(md) && /IMAGE:테마/.test(md),
    'CLAUDE.md 에 `CONTENT:테마`·`IMAGE:테마` 가 없습니다');
});
