...
test('★★ 감추는 목록이 앱 껍데기 **다섯**을 모두 덮는다', () => {
  for (const sel of ['.side', '.top', '.steps', '.bot', '.tabs']) {
...
/* ★★★ **머리는 감추지 않는다 — 탭만 감춘다** 〈2026-09-07 · 사장님 화면〉.
 *
 *   [사고] 캐시를 비우고 새 판을 보시자 **탭 줄이 두 벌**로 떴다 — 앱의
 *     「보고서 만들기 / 완성 보고서」 바로 아래에 이 화면이 그린 같은 것이 한 벌 더.
 *     어느 쪽을 눌러야 하는지 알 수가 없다.
 *
 *   [무엇이 어느 쪽 몫인가] CLAUDE.md §8-2 가 정해 두었다 —
 *     **머리는 이쪽 화면이 달고, 탭 줄은 앱 것이다.**
 *     그래서 이 검사는 **둘을 반대로 재면 빨개진다** — 머리를 감추면 앱 안에서
 *     화면이 이름을 잃고, 탭을 안 감추면 두 벌이 된다. 둘 다 실제로 겪었다.
 */
...
test('★★★ 화면이 실제로 머리와 탭을 그린다 — 안 그러면 위 두 칸이 헛돈다', () => {
  /* ★ 소스를 보지 않는다. 머리와 탭은 **스크립트가 만든다** — `report-flow.html` 의
     소스에는 그 글자가 한 번도 안 나온다. 실제로 여기서 한 번 빨개졌다.
     그래서 **미리 그려 넣은 판**(헤드리스로 진짜 그려서 만든 것)에서 센다. */
  const art = fs.readFileSync(path.join(P, 'section-artifact.html'), 'utf8');
  assert.ok(/class="head"/.test(art),
    '★ 그려진 결과에 머리가 없다 — 앱은 §8-2 대로 제 배너를 지우므로 아무도 이름을 안 단다');
  assert.ok(/class="tabs"/.test(art),
    '★ 그려진 결과에 탭 줄이 없다면 `.tabs` 규칙은 재는 것이 없다 — 규칙을 지우거나 이 검사를 고친다');
});
