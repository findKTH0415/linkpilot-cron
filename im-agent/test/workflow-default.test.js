...
test('★★ 앱이 쓰는 길로도 재고, 못 쟀으면 못 쟀다고 말한다', () => {
  ...
  /* ★★★ **다르다고 배포를 세우지는 않는다** 〈2026-08-27 · 실제로 세웠다〉.
   *
   *   앞 판은 하나라도 다르면 `exit "$BAD"` 였다. 그래서 배포 #148·#149 가
   *   여기서 죽고 **열쇠 파일·엔진 배포가 통째로 건너뛰어졌다** — 올리기와
   *   「닿았는가」는 이미 초록이었는데도 배포 전체가 빨갛게 끝났다 (M-60 과 같은 결).
   *
   *   ★ 여기서 다른 것은 **이 배포가 못 고치는 것**이다: 앱 사본은 사람이
   *     올리고(D-143), 앱이 다른 폴더를 보면 그것은 앱 쪽 설정이다.
   *     배포를 세워도 둘 중 어느 것도 나아지지 않는다.
   *   ★ 그래서 **크게 적고 넘어간다.** 문턱은 `Verify deployed` 하나다.
   *   ★ 다만 **조용해지지는 않는다** — 몇 개가 다른지와 무엇을 뜻하는지를 적는다. */
  assert.ok(!/exit "\$BAD"/.test(s), '어긋난 개수로 배포를 세운다 — 뒤 단계가 통째로 건너뛰어진다');
  assert.match(s, /\n\s*exit 0\s*\n/, '마지막이 exit 0 이 아니다');
  assert.match(s, /개 다름/, '몇 개가 다른지 안 적는다 — 조용해지면 없느니만 못하다');
  assert.match(s, /이 배포로는 못 고친다/, '왜 안 세우는지를 안 적는다');
});
...
test('★★★ Secret 이 있으면 배포가 열쇠 파일을 대신 놓는다', () => {
  ...
  /* ★★★ **재는 자리를 옮겼다** 〈2026-09-17 · D-212〉 — 이제 그 자리에 바로 안 쓰고
   *   임시 이름에 쓴 뒤 `mv` 로 갈아 끼운다(옛 파일 소유자가 달라 덮기가 거부됐다).
   *   `mv` 는 새 파일의 모드를 가져오므로 성질은 그대로다 (§6-2-5 의 잣대). */
  assert.ok(/chmod 600 '?\.?linkpilot\.env/.test(step),
    '열쇠 파일이 될 것의 권한을 안 좁힌다');
