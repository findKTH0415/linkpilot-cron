...
test('★ 쓰이지 않는 문체가 셋 있고, 그것이 정상이다', () => {
  // research·legal_review·court 는 맞는 테마(연구·로펌·법원)가 아직 없다.
  // 없는 테마에 문체만 붙이면 쓸 수 없는 짝이 생긴다 — D-129 로 올렸다.
  ...
  assert.deepStrictEqual(unused.sort(), ['court', 'legal_review', 'research'],
    '쓰이지 않는 문체가 달라졌다 — 테마를 더했으면 D-129 를 갱신하라');
});
