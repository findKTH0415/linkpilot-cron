...
/* ★★★ G. **잰 값이 왔는데 표가 「미측정」이라 말하면 그 표가 옛말을 한다**
   〈2026-09-20 · D-247〉.
   [왜] `WEATHER_GO` 는 2026-09-13 부터 「어느 기관인지 아직 안 쟀다」로 적혀 있었다.
   2026-09-20 에 `scripts/weather-probe.mjs` 가 **판정 0**(공공데이터포털 기상청)을 냈는데,
   그 줄을 안 고치면 **없는 장애가 유령으로 남아** 사장님께 「아직 못 쟀습니다」라고
   말하게 된다 (M-86 · §4.6 「이미 하신 일을 또 하시게 만든다」와 같은 결).
   ★ **이 칸은 이름을 손으로 안 적는다** — 결과 파일이 말하는 열쇠를 읽어 그 줄을 본다.
     다음에 다른 열쇠를 재게 되는 날 **자동으로 걸린다** (§8-1).
   ★★ **결과 파일이 없거나 판정이 0 이 아니면 「미측정」이 옳다** — 그때는 이 칸이
     아무것도 안 잰다고 적고 넘어간다. **못 잰 것을 실패로도 통과로도 뭉뚱그리지 않는다** (§8). */
test('G. 진단이 «판정 0» 을 낸 열쇠를 표가 「미측정」이라 적지 않는다 (D-247)', () => {
  const MD = path.join(ROOT, 'data', '_api', 'weather-probe.md');
  if (!fs.existsSync(MD)) {
    console.log('  · 진단 결과(weather-probe.md)가 없다 — 이 칸은 아무것도 안 잰다 (§8)');
    return;
  }
  const md = read(MD);
  const head = md.split('\n').find((l) => l.startsWith('>')) || '';
  if (!/판정\s*0\b/.test(head)) {
    console.log(`  · 아직 판정 0 이 아니다 — 「미측정」이 옳다: ${head.slice(0, 80)}`);
    return;
  }
  /* 결과 파일이 말하는 열쇠 이름을 읽는다 — 손으로 안 적는다 */
  const m = md.match(/열쇠\s*\*\*`([A-Z0-9_]+)`\*\*\s*로 읽었다/);
  assert.ok(m, '결과 파일에서 «어느 열쇠를 쟀는지»를 못 읽었습니다 — 이 칸이 아무것도 안 잽니다');
  const key = m[1];

  const { KEYS } = require(path.join(ROOT, 'im-agent', 'tools', 'api-report.js'));
  const row = KEYS.find((r) => r[0].split('|').includes(key));
  assert.ok(row, `진단 표에 ${key} 줄이 없습니다 — 재 놓고 표에 안 올렸습니다`);
  assert.ok(!/미측정|안 쟀다|미확인/.test(`${row[1]} ${row[2]}`),
    `${key} 는 진단이 **판정 0** 을 냈는데 표가 아직 「미측정」이라 적습니다 —\n`
    + `  없는 장애가 유령으로 남아 사장님께 이미 하신 일을 또 하시라고 말합니다 (M-86).\n`
    + `  판정: ${head.trim()}\n  표   : ${row[1]} | ${row[2]}`);

  /* ★ 그리고 결과가 «어느 기관»이라고 말한 그 낱말이 표에도 있어야 한다 —
     「미측정만 아니면 된다」로 두면 엉뚱한 기관을 적어 놓아도 통과한다. */
  const org = head.match(/\*\*([가-힣A-Za-z ]+)\*\*/);
  if (org) {
    assert.ok(`${row[1]} ${row[2]}`.includes(org[1].trim()),
      `판정이 「${org[1].trim()}」이라 말하는데 표는 「${row[1]}」입니다 — 두 자리가 다른 말을 합니다 (§8)`);
  }
});
