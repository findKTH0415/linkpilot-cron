...
/**
 * ★ 처음 12종은 사용자가 말한 목록이고, **에너지 4종(태양광·ESS·해상풍력·
 *   육상풍력)은 2026-08-16 에 사용자가 더한 것이다** (등록부 D-55·D-56·D-58).
 *   늘어난 것을 조용히 넘기지 않으려고 여기에 경위를 적는다 — 목록만 고치면
 *   반년 뒤 "왜 16종이지"가 된다.
 ...
 */
test('★ 자산군 16종이 전부 있다 (12종 + 에너지 4종)', () => {
...
/**
 * ★ 목록을 화면에 적어 두면 자산군을 늘리는 날 **화면만 옛말**을 한다.
 *
 * ★★ 〈2026-08-22〉 `report-flow.html` 은 이제 자산군을 **그리지 않는다** —
 *   그것을 보여 주던 「새보고서 진행률」 절을 사용자 지시로 뺐다. 그래서 이
 *   검사는 「베껴 쓰지 않는다」를 **더 강하게** 잰다: 이름도, 서버에서 받아
 *   오던 자리도 없어야 한다. 되살아나면 그때 이 검사부터 손보게 된다.
 */
test('★ 화면이 자산군 목록을 베껴 쓰지 않는다', () => {
  ...
  assert.ok(html.indexOf('assetClass') === -1,
    'report-flow 가 자산군을 다시 그리고 있다 — 그렇다면 목록을 어디서 받는지부터 본다');
});
