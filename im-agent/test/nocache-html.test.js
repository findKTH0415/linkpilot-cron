 ...
 *
 * 앞 판은 Cache-Control 값이 비면 다음 길로 물러났다. 그런데 **헤더가 없는
 * 것이 정상 결과**다 — 안 켜져 있으면 당연히 빈다. 그래서 앱이 쓰는 길을
 * 멀쩡히 재고도 「못 쟀다」로 보고 8080 으로 물러났고, 판정에 `포트 8080` 이
 * 찍혔다.
 *
 * ★ 사장님이 헤더를 다신 자리는 역방향 프록시(`https://*:843`)다. 8080 을
 *   재는 한 판정은 **영영 「안 켜짐」**이고, 그러면 켜 두고도 안 켜졌다고
 *   믿게 된다 — M-25 와 같은 결이다.
 */
test('★★★ 앱이 쓰는 길은 **응답을 받았는지**로 갈린다 (헤더 유무가 아니라)', () => {
  const step = STEP;
  assert.match(step, /GOT=1/,
    '응답을 받았다는 표시가 없다 — 헤더가 비면 딴 길로 물러난다');
  assert.match(step, /if \[ -z "\$GOT" \]; then/,
    '물러나는 조건이 「응답을 못 받았을 때」가 아니다');
  assert.ok(!/if \[ -z "\$CC" \]; then\s*\n\s*for PORT/.test(step),
    'Cache-Control 이 비었다고 딴 포트로 물러나고 있다 — 안 켜진 것을 못 잰 것으로 읽는다');
});

test('★★ 앱이 쓰는 길을 못 쟀으면 판정에 그렇게 적는다', () => {
  const step = STEP;
  assert.match(step, /앱이 쓰는 길은 못 쟀다/,
    '딴 포트로 갈린 사실이 판정에 안 적힌다 — 「포트 8080 에서 안 켜짐」은 '
    + '사장님 화면에 대한 답이 아니다');
});

/**
 * ★ 이 NAS 는 Nginx 다(2026-08-28 실측). 그러므로 안내가 `.htaccess` 를
 *   가리키면 **없는 길로 보낸다.**
 */
test('★ 안 켜졌을 때의 안내가 실제로 켤 수 있는 자리를 가리킨다', () => {
  const step = STEP;
  assert.match(step, /역방향 프록시/,
    '켜는 자리(역방향 프록시)를 안 가리킨다');
  assert.match(step, /Nginx 라 올린 \.htaccess 는 읽히지 않는다/,
    '.htaccess 가 이 NAS 에서 무용이라는 실측이 안 적혀 있다');
});
