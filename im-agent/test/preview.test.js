 ...
 *   다음을 찾는다 — 찾을 것이 없다.
 */
test('단계는 6개이고 순서·이름이 고정되어 있다', () => {
  /* ★ 「완성 보고서」가 흐름 안으로 들어와 여섯이 되었다 〈2026-08-28 · D-157〉.
   *   앞 판은 딴 탭이라, 만들고 나면 사용자가 갈 곳을 스스로 찾아야 했다. */
  assert.deepStrictEqual(
    SCREENS.map(s => `${s.no}. ${s.name}`),
    ['1. 제작 기본정보 입력', '2. 무엇을 만들까요?', '3. 관련자료 업로드',
      '4. 가이드 필드 (자동입력 + 직접입력)', '5. 출력조건', '6. 완성 보고서']);

  // 번호는 1..6 으로 이어져야 한다 — 건너뛰면 흐름이 아니라 목록이 된다
 ...
 */
test('단계 이름을 화면이 **읽어 온다** (베껴 쓰지 않는다)', () => {
  /* ★★ 앞 판은 `reports.html` 이 이름 여섯을 손으로 적어 두었고, 이 검사는
     그 배열을 찾아 글자를 맞췄다. 그런데 레일의 마지막 이름이 바뀌자
     **화면만 옛 이름을 말했다** 〈2026-08-28 · D-157〉.
     ★ 그래서 「글자가 같은가」 대신 **「읽어 오는가」**를 잰다 — 읽어 오면
       갈릴 수가 없고, 다음에 이름을 바꿔도 이 검사를 고칠 일이 없다. */
  const html = fs.readFileSync(path.join(PLATFORM, 'reports.html'), 'utf8');
  assert.match(html, /FL\.STEPS\.map\(function \(st\) \{ return st\.name; \}\)/,
    'reports.html 이 단계 이름을 flow-core 에서 읽지 않는다 — 사본은 갈린다');
  SCREENS.forEach((sc) => {
    assert.ok(html.indexOf("'" + sc.name + "'") === -1,
      `reports.html 이 「${sc.name}」 을 직접 적고 있다`);
  });
});
