...
test('★★★ 장치를 빼면 **화면이 안 뜬다** (통과가 아니라 실패를 잰다)', (t) => {
  ...
  try {
    ...
    /* ★★★ **표본이 섰는지 먼저 본다** 〈2026-09-07 · CI 에서 간헐적으로 빨갰다〉.
     *
     *   [사고] 같은 커밋인데 한 실행은 초록, 한 실행은 빨갰다. 빨간 쪽이 하는 말은
     *     「장치를 빼도 화면이 그대로 뜬다」였는데, **실제로는 멀쩡한 자리마저
     *     안 그려진 것**이었다. `btn(b)` 가 0 이면 `0 < 0` 이 거짓이라 이 칸이
     *     **엉뚱한 것을 탓한다.**
     *
     *   ★ 위 시험(정상 경로)에는 이 확인이 이미 있다 — 「멀쩡한 자리에서도 안
     *     그려진다 — 표본이 거짓말을 한다」. 사보타주 칸에만 빠져 있었다.
     *     **빨간 줄을 보면 표본부터 의심한다** (CLAUDE.md §8).
     *
    ...
    const FLOOR = 5;
    if (btn(b) < FLOOR) {
      t.skip('멀쩡한 자리가 버튼 ' + btn(b) + '개에서 끊겼다 (다 서면 9개) — 표본이 안 서서 '
        + '**못 쟀다** (통과가 아니다). 렌더가 예산(12초) 안에 못 끝났을 수 있다');
      return;
    }
