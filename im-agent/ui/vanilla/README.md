...
`Lineage.jsx` 를 이 판에도 옮겼다 (2026-08-15). 없을 때는 **숫자를 눌러 출처를
확인하는 경로가 통째로 막혀 있었다** — 출처 없는 숫자를 막는 것이 이 시스템의
전부인데, 화면에서 출처를 볼 수 없으면 그 규칙이 사람에게는 보이지 않는다.

```js
var ct = LinkPilotControlTower.mount(box, { projectId: 'LP-DC-2026-001' });
ct.openLineage('building.gfa_sqm');   // 본체가 개별 숫자에 붙일 수 있다
```

- 바닥의 [데이터 계보 보기] 버튼은 `opts.lineageKey` 로 바꾼다
- ESC · 바깥 클릭 · 닫기 버튼 셋 다 닫힌다
