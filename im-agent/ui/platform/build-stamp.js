 ...
 *     브라우저가 **다시 받을 수밖에 없다.**
 */
/* ★★★ **미리받기(`<link rel="preload">`) 주소도 «함께» 지문을 단다** 〈2026-09-15
 *   사장님 지시: 「보고서 생성 … 아직도 느리고 늦게 로딩됨」〉.
 *   [왜 반드시 함께인가] 미리받기 주소의 `?v=` 가 스크립트 주소와 **한 글자라도 다르면**
 *   브라우저는 **다른 파일로 본다** — 같은 파일을 두 번 받아 **오히려 느려진다.**
 *   ★ 그래서 재는 자리(`--check`)도 같은 자를 쓴다: 셋 중 하나가 옛 판이면 빨갛다. */
const VER = /(<script src="|<link rel="stylesheet" href="|<link rel="preload" as="script" href=")([^"?]+)(\?v=[0-9a-f]*)?"/g;
