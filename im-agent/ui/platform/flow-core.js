...
(function (root, factory) {
  ...
  var EMBED_CSS = [
     ...
     *
     *   [무엇이 어느 쪽 몫인가] CLAUDE.md §8-2 가 정해 두었다 —
     *     · **머리(제목·부제)는 이쪽 화면이 단다.** 그래서 `.head` 는 **안 감춘다.**
     *     · **탭 줄은 저쪽(앱) 것이다.** 그것은 배너가 아니라 **아래 틀의 주소를
     *       바꾸는 동작**이라 앱에 있어야 한다 — 지우면 완성 보고서로 갈 길이 없어진다.
     *
     *   ★ 그러니 이쪽 탭은 **혼자 열렸을 때만** 뜬다. 이 CSS 는 틀 안일 때만 얹히므로
     *     단독 화면에서는 그대로 보인다 — 그때는 견줄 앱 탭이 없어 이것이 유일한 길이다.
     *   ★★ 지우지 않고 **감추는** 이유가 그것이다. 지우면 단독으로 열었을 때
     *     「완성 보고서」로 갈 문이 사라진다 (§8-2 가 저쪽에서 그것을 이미 한 번 겪었다). */
