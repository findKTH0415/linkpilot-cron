...
function diagnose(status, body, transportError) {
  ...
  /* ★★★ **「신청 전」과 「신청했는데 아직」은 «같은 응답»으로 온다** 〈2026-09-14 · 사장님: 「활용신청 했음」〉.
     [무엇이 났나] 앞 판의 글은 「신청한다」 하나뿐이었다. 그런데 사장님이 **이미 신청하신 뒤**에
       그 글을 보시면 **또 신청하시게 된다** — 바로 위 주석이 경계한 그 실수(M-86)를
       이 갈래가 그대로 저지르고 있었다.
     ★ **코드는 신청 여부를 모른다.** data.go.kr 은 「신청 안 함」과 「승인·전파 대기」에
       **똑같은 코드(30)**를 준다 (§4.2). 그러니 갈라 «판정»할 수는 없다.
     ★★ 그래서 **두 경우를 한 글에 함께 적는다.** 판정할 수 없는 것을 판정한 척하지 않고,
       읽는 분이 **자기가 어느 쪽인지 알므로** 그 자리에서 갈린다.
     ★★★ **다시 재는 법을 함께 적는다.** 「기다린다」만 적으면 언제 어떻게 다시 보는지가
       안 보여, 기다리는 동안 아무도 확인을 안 한다 (§5 「그러면 어떻게 되는지」). */
  if (/SERVICE_KEY_IS_NOT_REGISTERED_ERROR|등록되지\s*않은/.test(t)) {
    return { kind: 'approval',
      head: '이 API 가 **아직 이 키로 안 열린다** — 키 문제가 아니다 (§4.2). 둘 중 하나다: '
        + '① 아직 활용신청 전이면 data.go.kr 에서 「특일정보」(B090041)를 신청한다 · '
        + '② **이미 신청하셨다면 승인·전파를 기다리는 중이다 — 다시 신청하지 않는다** '
        + '(대개 즉시~1시간, 담당자 승인이 붙는 서비스는 1~2일). '
        + '다시 재는 법: DSM 「작업 스케줄러」 → `calendar-nas` → [실행] → [작업] → [결과 보기]. '
        + '이 줄이 사라지고 받은 해가 0 이 아니면 열린 것이다' };
  }
