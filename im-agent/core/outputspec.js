...
const VISUAL_DEFAULT = {
  /**
   * ★★★ **지적도면** 〈2026-08-25 사장님 지시: 「출력시 옵션이 [지적도면],
   *   [조감도] 반영할시 SketchUp Engineering Agent 로부터 불러오도록」〉.
   *
   *   필지 경계·지번·면적을 그린 도면이다. **조감도와 다른 것**이라 따로 둔다 —
   *   조감도는 건물이 선 그림이고, 지적도면은 **땅만** 그린 것이다. 하나로
   *   묶으면 「땅만 필요한데 건물까지 그려진」 문서가 나온다.
   *
   * ★ 켜면 **SketchUp 쪽에 만들어 달라고 적는다** (`model-plan.json` 의
   *   `deliverables`). 엔진이 대신 그리지 않는다 — 엔진이 그리면 SketchUp 판과
   *   두 벌이 되고, 그때 어느 것이 문서에 실렸는지 알 수 없게 된다.
   */
  cadastral: true,
