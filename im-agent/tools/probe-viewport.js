...
/** 어느 폭에서 보는가
 *  ★ 2026-09-12 — **좁은 폰과 큰 폰을 더했다** 〈업무지침서 v1.1 §11.3: 360·390·412〉.
 *    앞 판은 390 하나였다. 그런데 **잘리는 것은 가장 좁은 자리에서 먼저 난다** —
 *    360(갤럭시 S 기본·작은 안드로이드)에서 나는 넘침은 390 에서는 안 보인다.
 *    412(갤럭시 S 큰 화면·Pixel)는 반대로 **가로가 남아** 자리가 어색해지는 쪽을 잡는다.
 *  ★★ 재는 폭이 늘면 도는 시간도 는다. 그래도 두는 이유는, 이 저장소에서 난 화면 사고
 *    둘이 **폰에서 잘린 것**이었고 둘 다 사장님이 눈으로 찾으셨기 때문이다 (M-72). */
const SIZES = [
  { name: '좁은 폰', w: 360, h: 800 },
  { name: '폰', w: 390, h: 844 },
  { name: '큰 폰', w: 412, h: 915 },
  { name: '맥', w: 1440, h: 900 },
];
