...
function aheadNone(dir, refs) {
  ...
  /* ★★★ **«여럿이 하나로» 스쿼시되는 경우를 한 번 더 막으려다 «되돌렸다»**
       〈2026-09-21 · 실측 · §12-29 의 그 잣대〉. `diff --quiet` 로 트리를 통째로
       견주는 한 겹을 넣었는데, **사보타주로 빼 보니 13칸이 전부 초록이었다** —
       곧 **그것이 무엇을 고쳤는지 못 쟀다.** 아래 «ref 마다 돌려 교집합» 이 그 일을
       이미 하고 있었다(실측: 여섯이 아니라 1 개로 나왔다).
     ★ **추측으로 넣은 것은 원인이 아니라고 판명되면 지운다** — 남겨 두면 다음 사람이
       그것을 원인으로 읽는다. 대신 **그 상황을 못박는 검사 둘**은 남긴다. */
  let keep = null;
  for (const r of live) {
