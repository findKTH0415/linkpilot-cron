 ...
 * 남기는 것: 문서 뿌리(`documentElement`)에 **결과 표시**를 하나 붙인다 — 칸마다
 *   `{rel, ok, why}` 를 담은 배열이다. 표시 이름은 아래 진짜 코드에만 있다.
 *   ★★★ **여기에 그 이름을 글자로 적지 않는다** 〈2026-09-13 · 실측으로 당했다〉.
 *     이 파일은 페이지 안에 **통째로 심어지므로** 주석도 DOM 에 남는다. 읽는 쪽이
 *     처음 나오는 것을 집으면 **주석을 읽고** `[…]` 를 JSON 으로 풀려다 죽는다 —
 *     실제로 그래서 이 칸이 「못 쟀다」로 조용히 서 있었다 (CLAUDE.md §8 · M-84).
 */
(function () {
