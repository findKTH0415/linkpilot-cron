...
const KEYS = [
  ...
  /* ★★★ `WEATHER_GO` 는 **공공데이터포털 기상청**이다 〈2026-09-20 실측 · D-247〉.
     `scripts/weather-probe.mjs` 가 네 후보를 걸어 갈랐다 — API허브는 **401 「유효한
     인증키가 아닙니다」**로 거부했고, 포털의 `VilageFcstInfoService_2.0` 가
     `resultCode 00 NORMAL_SERVICE` 와 `obsrValue` 를 돌려줬다.
     ★ **이름으로 추측했으면 거꾸로 갔다** — `weather.go.kr` 은 기상청을 가리키지만
       기상청은 열쇠를 받는 자리가 둘이고 인증 방식이 서로 다르다 (§4.3).
     ★★ **`datakey.js` 에 안 넣는다** — 포털 열쇠이긴 해도 **승인은 서비스 단위**라
       (§4.2) 이 열쇠가 단기예보에 승인된 것이 특일정보·실거래가 승인을 뜻하지 않는다.
       넣으면 그 이름이 앞자리를 차지해 **멀쩡히 돌던 조회가 조용히 거부될 수 있다.** */
  ['WEATHER_GO', '공공데이터포털 기상청', '단기예보 (VilageFcstInfoService_2.0) — 아직 커넥터에 안 붙였다'],
