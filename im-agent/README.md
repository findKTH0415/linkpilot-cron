...
이 저장소는 보고서 생성 전용이다 — 아침 브리핑 크론은 2026-08-14 삭제되었다
(등록부 D-18 · 옮길 곳은 D-19 로 미결정).
...
| 좌표 → 필지 폴리곤·PNU | VWorld 연속지적도 | 180일 |
| 공부상 대지면적·지목 | VWorld 토지특성 | 30일 |
| 용도지역·지구 전체 → 용적률/건폐율 상한 | VWorld 토지이용계획 | 30일 |
| 기존 건축물 연면적·건폐율·층수·대지면적 | 건축물대장(건축HUB) | 30일 |
| 연간 일사량·일조시간 〈태양광〉 | 기상청 지상관측 일통계 | 30일 |

> **연속지적도에는 면적 필드가 없다.** 대지면적은 건축물대장 `platArea` 가
> 채우고, 나대지(건축물대장 없음)는 토지특성이 채운다. 개발사업 부지는 대개
> 나대지라 실사용에서는 토지특성 경로가 주로 쓰인다.

프로젝트당 07 Geo 는 4~6회 호출한다. 파이프라인 전체(08 감정평가의 실거래가
6개월·부동산원 지역색인, 05 검증의 인허가·법인 조회 포함)로는 약 25회다.
전부 캐시되므로 **재실행 시 0회**다.
...
| 방식 | 산정 | 데이터 |
|---|---|---|
| 공시지가 기준 | 개별공시지가 × **시점수정** × 면적 × 현실화계수 | VWorld NED + 부동산원 지가지수 |
...
- **캐시 히트는 네트워크도 쿼터도 소모하지 않는다.** 데이터 성격별 TTL을 둔다
  (지오코딩·지적·부동산원 지역색인 180일 / 공시지가·용도지역·지가지수·일사량
  30일 / 실거래가·REC 7일).
- **기관마다 키가 다르다.** VWorld · data.go.kr · 기상청 API허브 · 한국부동산원
  네 곳이고 서로 통용되지 않는다. 커넥터도 그 경계로 나뉜다
  (`vworld`/`nsdi` · `molit`/`kpx`/`fsc` · `kma` · `reb`).
- **응답이 오름차순으로 오는 API 가 많다.** 개별공시지가·토지특성·REC·지가지수가
  그랬다. 앞에서부터 집으면 몇 년 전 값을 최신으로 쓰게 되고, 출처 표시는
  멀쩡해서 문서만 봐서는 안 잡힌다. 새 Connector 는 **정렬 방향부터 확인한다.**
...
엔드포인트·파라미터는 공식 문서 기준이지만 **응답 필드명이 다를 수 있다.** 그러면 조회는
성공하는데 값이 전부 null 로 들어온다 — 가장 잡기 어려운 종류의 오류다.

```bash
VWORLD_KEY=... DATA_GO_KR_KEY=... npm run im:smoke
```

각 API를 한 번씩 호출해 Connector 가 기대하는 필드가 실제 응답에 있는지 대조하고,
없으면 어느 필드명을 고쳐야 하는지 표시한다. 키는 출력에 찍지 않으며 아무것도 저장하지 않는다.

> VWorld 키는 신청 시 등록한 도메인에서만 동작한다. 서버에서 호출하려면 콘솔에
> 서버 IP/도메인을 등록하거나 `VWORLD_DOMAIN` 환경변수에 등록된 도메인을 넣어야 한다.
