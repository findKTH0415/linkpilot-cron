...
process.env.IM_AGENT_CACHE = process.env.IM_AGENT_CACHE
  || path.join(os.tmpdir(), `lp-wonju-${Date.now()}`);
...
const ADDRESS = '강원특별자치도 원주시 신림면 송계리 695-4';
...
if (!li.ok) {
  ...
  if (!rg.ok) {
    ...
    if (!ta.ok) {
      ...
      await writeFile(`${OUT}/wonju_index_monthly.json`, JSON.stringify(v.monthly, null, 2));
    }
  }
}
