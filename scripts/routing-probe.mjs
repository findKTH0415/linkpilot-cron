 ...
 *
 * ★★ **경로와 숫자만 담는다** (§2 · 이 저장소는 공개다 · D-10). 값이 **숫자인 칸만**
 *   적으므로 열쇠·개인정보가 실릴 자리가 없다. 경로도 `redact()` 를 지나간다.
 *
 * ★ **JSON 으로 못 읽으면 「못 쟀다」로 적는다** — 빈 것으로 두면 「그 칸이 **없다**」와
 *   같은 값이 되어, 이름만 다른 멀쩡한 응답을 「규격이 틀렸다」로 읽게 된다 (§8 · §12-12).
 */
function findFields(body, re, max = 4) {
  let root;
 ...
 */
async function probe(label, url, init, valueRe, fieldRe) {
  const t0 = Date.now();
  try {
