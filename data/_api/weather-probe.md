> 판정 0 — `WEATHER_GO` 는 **공공데이터포털** 기상청 열쇠다. 그 규격으로 배선한다

# `WEATHER_GO` 가 어느 기관 것인가 — 실측 진단

조회일 2026-09-21 · 기준 20260921 1000 (KST) · 서울

> 규격을 모르는 채 배선하지 않는다. 후보를 걸어 **무엇이 오는지부터** 본다 (CLAUDE.md §4).
> ★ 관측값이 **문자열**이라 숫자 칸 찾기(D-230)는 **안 건다** — 경로는 본문 앞머리로 읽는다.

- 열쇠 **`WEATHER_GO`** 로 읽었다 (길이 64자 · 값은 안 적는다)

## 1. 기상청 **API허브** (`apihub.kma.go.kr` · `authKey`)

- `typ01 지상관측 kma_sfctm2` — HTTP 401 · 1622ms
  - 본문 «{ "result" : { "status" : 401, "message" : "유효한 인증키가 아닙니다." } }»
  - 관측값 칸: **못 찾았다**
  - 서버 Apache
- `typ02 초단기실황 getUltraSrtNcst` — HTTP 401 · 1584ms
  - 본문 «{ "result" : { "status" : 401, "message" : "유효한 인증키가 아닙니다." } }»
  - 관측값 칸: **못 찾았다**
  - 서버 Apache

> **판정 4** — **인증이 거부됐다** — 열쇠 자체이거나 **그 서비스 등록·신청**이 안 된 것이다. ★ 상태코드가 **200 이어도** 본문이 그렇게 말하는 곳이 있다(ODsay 가 그렇다). 아래 응답 본문이 둘 중 어느 쪽인지 말해 준다

## 2. **공공데이터포털** 기상청 (`apis.data.go.kr/1360000` · `serviceKey`)

> ★ 이 호스트는 **러너에서 안 열릴 때가 있다** (D-206 · 2026-09-20 실측 재확인).
> 그때 판정 3 은 **열쇠 문제가 아니다** — 도는 자리를 NAS 로 옮겨 다시 잰다.

- `원본 그대로` — HTTP 200 · 875ms
  - 본문 «{"response":{"header":{"resultCode":"00","resultMsg":"NORMAL_SERVICE"},"body":{"dataType":"JSON","items":{"item":[{"baseDate":"20260921","baseTime":"1000","category":"PTY","nx":60,"ny":127,"obsrValue":"0"},{"baseDate":"20260921","baseTime":"1000","category":"REH","nx":60,"ny":127,"obsrValue":"46"},{» …
  - ★ 본문은 **앞 300자만** 적는다. 판정은 **본문 전체**로 했다 (D-228)
  - 관측값 칸: **찾았다**
- `한 번 디코딩` — 원본과 같아 건너뛴다

> **판정 0** — **값이 왔다** — 이 후보로 배선한다
