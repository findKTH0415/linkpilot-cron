...
**잰 시각** 2026. 9. 19. 14시 23분 23초 · **잰 곳** GitHub Actions (열쇠가 있는 자리)
...
| `VWORLD_KEY` 또는 `LINKPILOT_VWORLD_REPORT_KEY` 또는 `LINKPILOT_VWORLD_WEB_KEY` | 브이월드 | 지오코딩·지적·토지특성 (이름 셋 다 읽는다 — connectors/vworldkey.js) | ✅ `LINKPILOT_VWORLD_REPORT_KEY` |
...
| `ODSAY_API_KEY` 또는 `ODSAY_KEY` | ODsay | 대중교통 소요시간 (이름 둘 다 읽는다) | — **없음** |

★ **1개가 안 들어왔습니다.** Secrets 에 없거나 **이름이 다릅니다** —
