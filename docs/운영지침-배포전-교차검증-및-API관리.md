...
## A-1. 같은 이름이 다른 뜻으로 쓰인다 — **다섯 + Gate 번호** 🔴
...
| `테마` | 인사이트 지침서 §5.3.2 — **문장의 분류 60개** (성장·도전 …) | `linkpilot-platform` 의 `_IMG_TAGS` — **배경 사진 검색어 51개** | 하나는 글을 고르는 잣대, 하나는 그림을 찾는 낱말 |
| `다운로드` | 인사이트 지침서 §7.4 — **완성 카드 이미지 저장** (요구사항) | 사진 규칙 원칙 ③ — **원본 사진 내려받기** (금지) | 하나는 반드시 되어야 하고, 하나는 절대 안 된다 |
...
★★★ **「테마」와 「다운로드」는 2026-09-12 에 더해졌다** 〈인사이트 지침서 v1.5 교차검증〉.

- **「테마」는 숫자가 비슷해서 더 위험하다.** 지침은 60개, 코드는 51개다 — 「테마를 60개로
  맞춰라」를 이미지 쪽에서 읽으면 **엉뚱한 것을 아홉 개 만든다.** **정한 것**: `CONTENT:테마` · `IMAGE:테마`.
- **「다운로드」는 한쪽이 요구사항이고 한쪽이 금지사항이다.** 지침 §7.4 는 완성 카드를 저장하라고
  하고, 사진 규칙 ③ 은 원본 사진을 주지 말라고 한다. **둘 다 맞다** — 가르는 것은 무엇을
  내려받는가다. **정한 것**: `CARD:다운로드`(허용) · `PHOTO:다운로드`(금지).
  ★ 이것을 안 적으면 「다운로드 기능을 구현하라」가 **원본 사진 내려받기 단추**로 배선될 수 있고,
    그러면 라이선스가 그 자리에서 깨진다.

★★★ **그리고 이제 「Gate 번호」도 겹친다** 〈2026-09-12 · 인사이트 지침서 v1.1 §13 · 지금은 v1.5〉.

「오늘의 인사이트 업데이트·개선 업무지침서 v1.1」이 **Gate 0~6** 을 새로 둔다.
그 번호가 이 지침의 Gate 와 **다른 것을 가리킨다.**

| 번호 | 이 지침 | 인사이트 지침서 v1.1 §13 |
|---|---|---|
| Gate 3 | **Staging 실환경 검증** — 지금 한 칸도 못 잰다 (D-201) | **콘텐츠·디자인** — 출처·문체·브랜드·라이선스 검수 |
| Gate 4 | **독립 Release 검증** (릴리스 판정) | **통합·보안** |
| Gate 5 | — | **실제 기기** |
| Gate 6 | — | **배포 승인** |

★ 낱말이 아니라 **숫자**가 겹치는 것이라 `role-guide.test.js` 의 상태 낱말 검사에는
  안 걸린다. 그래서 **사람이 적어 둔다** — 「Gate 3 통과」라는 말이 두 뜻을 갖는 것은
  똑같기 때문이다.

**정한 것** — 번호를 바꾸지 않는다(두 문서 다 여러 곳에서 가리킨다). 대신
**어느 게이트인지 접두어로 부른다**: 인사이트 쪽은 `INSIGHT:Gate 3`,
이 지침 쪽은 `RELEASE:Gate 3`. `READY_TO_DEPLOY` 에 쓴 것과 같은 방식이다.

★★ **접두어 없이 「Gate 3」만 적힌 글은 이 지침 것으로 읽는다** — 이 지침이 먼저 있었다.
