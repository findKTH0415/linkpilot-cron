...
| `WORLDWIDE_NEWS` | 세계뉴스 — **규격 미확인** | 미확인 | 미확인 | ✗ |
...
★ **베이스 URL 과 인증 파라미터를 여기 추측으로 적지 않는다.** 열쇠를 받은 뒤 진단을 돌려
**응답 본문을 그대로 보고** 채운다(§「진단부터 짠다」). 지금 적어 두면 그것이 곧 거짓이 된다.
★★ **이 자리(개발 컨테이너)에서는 네 곳 모두 안 열린다** — 실측:
`apis.openapi.kakaomobility.com` · `api.odsay.com` · `apis.openapi.sk.com` ·
`naveropenapi.apigw.ntruss.com` 이 전부 응답 없음이고, 같은 자리에서 `api.github.com` 은 열린다.
곧 **이 컨테이너의 나가는 길이 막힌 것**이지 러너·NAS 에서 안 열린다는 뜻이 **아니다** —
D-206 의 「못 닿음을 승인 안 됨으로 적지 않는다」와 같은 결이다. **도는 자리를 먼저 잰다.**
★ 이름을 **둘씩** 등록한 이유: `ECOS_API_KEY`/`ECOS_BOK_KEY` 와 `LAW_OC`/`LAW_OPEN_DATA` 에서
이미 두 번 당했다 — 이름이 갈리면 **아무 오류도 안 나고 조용히 값이 죽는다.**

**세계뉴스 열쇠 — 이름은 «화면에서» 읽는다** 〈2026-09-13 · 실측〉. 말씀으로 받은 이름은 `WORLD_NES_KEY` 였는데 **비밀 목록에 그 이름이 없었다.** 실제로 들어 있는 것은 **`WORLDWIDE_NEWS`** 와 **`WORLD_NEWS_API`** 둘이다.

★★★ **「들었다」와 「들어 있다」는 다른 사실이다.** 말씀만 믿고 등록했으면 실제 열쇠 둘이 `SECRET_ENV` 밖에 남아 **그 값만 로그에 평문으로** 나갈 수 있었고(§2), 부르는 코드는 **빈 값을 읽고 조용히 죽었을 것**이다. 열쇠 이름은 **반드시 화면에서 확인한다.**

★ **둘이 같은 서비스인지 아직 모른다.** 하나는 발급용, 하나는 엔드포인트용일 수도 있다 — 진단으로 가른다.

★★ **규격은 아직 모른다.** 베이스 URL·인증 파라미터·호출 한도를 확인하지 않은 채 파라미터를 추측해 넣지 않는다(2-3 「진단부터 짠다」). 먼저 **열쇠가 있는 자리에서 응답 본문을 200자 이상 그대로 받아** 규격을 확인하고, 그 결과를 보고 커넥터를 배선한다. 그전까지 검증 칸은 `✗`(미측정)로 둔다 — **못 잰 것을 통과로 적지 않는다.**
