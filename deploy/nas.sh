...
#
# ★★★ **이제 이것은 두 번째 길이다** 〈2026-08-23 · D-88〉.
#
#   평소에는 `deploy-nas` 워크플로가 올린다 — `main` 에 닿으면 자동으로 돌고,
#   엔진은 `deploy/engine.sh` 로 올린다. **쓰는 이는 하나여야 한다** (D-84).
#
#   ★ 이 스크립트는 **그 길이 못 돌 때**만 쓴다 (Actions 장애·tailnet 문제).
#     같은 날 둘을 같이 쓰지 않는다.
#   ★ 여기에는 **자동 되돌림이 없다** — 안 살아나면 되돌리는 명령을 화면에
#     적어 주고 끝난다. 사람이 보고 있을 때 쓰는 길이라 그렇게 뒀다.
#     지켜보는 사람이 없는 자동배포 쪽은 `deploy/engine.sh` 가 스스로 되돌린다.
#
